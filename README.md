...
A frozen replication of Gatev, Goetzmann & Rouwenhorst (GGR, 2006) — "Pairs
Trading: Performance of a Relative-Value Arbitrage Rule" — extended
...
alive only during market turbulence? This is not a strategy pitch — every
parameter (12-month formation, 6-month trading, 2σ trigger) is inherited
from the 2006 paper and frozen before any result was seen (`PROTOCOL.md`,
ratified 2026-07-05). Every place a result deviated from what the protocol
predicted is logged, dated, and explained in `DEVIATIONS.md`, not quietly
patched.
...
- **The effect has shrunk, not disappeared.** Top-20/wait-one-day/committed
  ...
  conventional levels — the point estimate roughly a third of what it was,
  consistent with the protocol's own "decline completed" reading of H1.
...
- **The bid-ask bounce mostly evaporated, in the expected direction, but
  ...
  small, in the predicted direction, but the bootstrap CI includes zero in
  both OOS sub-periods.
- **H3 did not confirm its own prediction.** The protocol predicted the
  ...
  their post-1988 decline. Reported as observed, not forced to match.
...
- **Two universes, always both, for different reasons:**
  ...
  - **Full point-in-time universe** — every actual S&P 500 constituent at
    ...
    basis for Gate 2 (`DEVIATIONS.md`, Gate 0 entry): the golden set was
    always meant as a fidelity check, not the main OOS universe. It is
    re-run as an explicit robustness arm alongside the full universe.
- **Falsification tests (not optional):** decile-matched random-pair
  ...
  same prior-month return decile — expected ≈0 or negative), long/short
  5-factor alpha decomposition, and a standard 5-factor (Mkt-RF, SMB, HML,
  Momentum, ST-Reversal) excess-return regression with Newey-West(6)
...
- **Gate 1** (2003–2009) validates replication fidelity against Do & Faff's
  ...
  re-execution after a data-quality fix (below), never silently re-run.
...
existed is smaller still since 2010 — consistent with "the decline
...
when volatility spikes — exactly the pattern GGR describe in the weak
markets of the 1970s and Do & Faff confirm in 2000–02 and 2007–09.
...
numbers sit close to GGR's *pre-1989* level, not their post-1988 decline —
the common factor behind pairs-trading returns looks just as present as it
did decades ago, even though the *returns* it's associated with have
shrunk (H1). See Figure 2. Reported as observed; the protocol's own
prediction was wrong here, and that's worth knowing rather than smoothing
over.
...
means — no bootstrap SE available for that row (see Figure 3, hatched bar).
...
Two things are true at once here, and both are reported rather than
picked: (1) the 2003–2009 delta is **negative** — wait-one-day beat
same-day, the opposite of GGR's expectation, an anomaly investigated and
explained in `DEVIATIONS.md` (persistent signals keep drifting before
reverting; entering a day later avoids that residual move); (2) both OOS
sub-periods flip to the **expected sign** and land well under the
predicted 10 bp/month ceiling — consistent with a mostly-collapsed
microstructure cost — but the confidence intervals include zero, so this
is a small, correctly-signed point estimate, not a statistically
established one.
...
This section is not a footnote. Three of the four items below were only
found because the frozen protocol requires reporting anomalies rather than
tuning them away.
...
delisting date — it deletes the ticker's entire history, including for
names later relisted or reused. Point-in-time universe attrition (share of
that year's S&P 500 membership with a complete Yahoo history), full table
in `results/replication/attrition.csv`:
...
carries this bias honestly rather than hiding it.
...
with replicate means around **10¹⁸ %** — a clear sign something was
numerically broken, not a strategy result. Root cause: **25 tickers** with
...
contamination turned out to be the second kind (a price frozen bit-identical
for months, which produces zero daily return and so never trips the first
filter, but fakes a low SSD against anything else that barely moves).
Both filters are strictly causal: they only ever inspect the current run's
own formation window, never future data — verified with dedicated tests
(`tests/test_formation.py`). The full-universe arm of Gate 2 was re-run
once after the fix, **explicitly logged as a second execution**, with the
pre-fix numbers preserved alongside the corrected ones
(`gate2_results.json`, key `full_universe_before_fix`) rather than silently
overwritten. Full writeup: `DEVIATIONS.md`, 2026-07-05 entries.
...
the pre-compounding daily-portfolio level) — the divergence appears only
after monthly compounding, typically a few bp/month. Documented in
`src/inference.py` and in both `gate1_report.md`/`gate2_report.md`.

### Gate 1's control-beats-top-20 divergence: cause identified, not fully decomposed

In the replication window, wait-one-day outperforms same-day on top-5/top-20
(reversed vs. GGR) and the control portfolio (101–120) outperforms top-20
— both on portfolio return and on payoff per trade (2.45×–3.02×, against a
formation-sigma ratio of only ~1.4×). Pair-by-pair diagnostics ruled out an
implementation bug (SSD monotonic, mechanics identical day-by-day,
no return duplication). The likely driver is the golden set's own
construction — requiring complete survival in *every* run 2003–2009 tends
to compress the top-20 toward highly correlated, low-spread mega-caps —
but this explains the *direction*, not the full *magnitude*, of the
divergence. Left as an open, dated item in `DEVIATIONS.md` rather than
forced closed.
...
multiple-testing correction. The intended message is not "clustering makes
more money" but "clustering finds fewer false discoveries at the same
performance" — discovery quality (OOS spread stationarity, convergence
rate) as the primary comparison metric, not raw returns.
...
Gate 0 (data) — point-in-time universe, price cache, golden sets:
```bash
