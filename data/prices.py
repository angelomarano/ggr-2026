"""
prices.py — Price download (RUN LOCALLY) + attrition report (Gate 0).

Usage:
    python -m data.download            # download everything (hours, resumable)
    python -m data.attrition           # generate results/replication/attrition.csv

Codified decisions (protocol §1):
- Primary source: Yahoo (`Adj Close` = total return proxy, required for the GGR
  normalized index). A ticker WITHOUT Yahoo Adj Close = declared attrition.
- Stooq: validation/spot-check ONLY (split-adjusted but not dividend-adjusted:
  mixing it into matching would change the return definition).
- Every failure goes into data/raw/download_failures.csv: that file IS the raw
  material for the attrition table, not an error to hide.
"""
...
EXTRA_SYMBOLS = ["^GSPC", "^VIX"]  # market and H2 regime
BATCH = 40
MAX_RETRIES = 3
...
def download_all() -> None:
    ...
    print(f"{len(tickers)} total symbols, {len(todo)} to download (resumed from cache)")
    ...
    for i in range(0, len(todo), BATCH):
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                print(f"  batch {i//BATCH}: attempt {attempt} failed ({e}); retrying...")
        ...
        for t in chunk:
            try:
                ...
                if "Adj Close" not in df.columns or df["Adj Close"].dropna().empty:
                    raise ValueError("missing Adj Close (no total return)")
                _save(t, df)
            except Exception as e:  # noqa: BLE001
                failures.append({"ticker": t, "error": str(e)})
        print(f"  batch {i//BATCH + 1}/{(len(todo)-1)//BATCH + 1} completed")
        time.sleep(2)

    pd.DataFrame(failures).to_csv(config.FAILURES_LOG, index=False)
    print(f"failed: {len(failures)} -> {config.FAILURES_LOG}")


# --------------------------------------------------------------------------
# Attrition (Gate 0): for each monthly run, the share of the point-in-time
# membership with COMPLETE history in the formation period (all reference
# trading days present, valid Adj Close, Volume > 0 — the GGR "no-trade days"
# filter).
# --------------------------------------------------------------------------
...
def attrition_report() -> pd.DataFrame:
    ...
    print("\nYearly average share_complete:\n", yearly.round(3).to_string())
    print(f"\nGate 0 clause threshold: {config.ATTRITION_MIN_COMPLETE_SHARE} over 2003-2009")
    return out
